// @generated by `source_sdk_2013_bindgen`. DO NOT EDIT.
// Regenerate with the `source_sdk_2013_bindgen/generate_bindings` Cargo feature.

#![allow(
    clippy::all,
    ambiguous_glob_reexports,
    non_snake_case,
    non_camel_case_types,
    non_upper_case_globals,
    unsafe_op_in_unsafe_fn,
    unused_imports,
    unused_qualifications
)]
#[allow(unused_imports)]
use super::super::super::__bindgen_prelude::*;
#[repr(C)]
#[derive(Debug, Copy, Clone)]
pub struct ServerClass {
    pub m_pNetworkName: *const ::std::os::raw::c_char,
    pub m_pTable: *mut SendTable,
    pub m_pNext: *mut ServerClass,
    pub m_ClassID: ::std::os::raw::c_int,
    pub m_InstanceBaselineIndex: ::std::os::raw::c_int,
}
include!("layout_assertions.rs");
